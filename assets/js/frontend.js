...
(function($) {
    ...
    const SCL = {
        ...
        bindEvents: function() {
            ...
            $(document).on('input', '#scl-search-input', this.handleSearch.bind(this));
            $(document).on('focus', '#scl-search-input', this.showSuggestions.bind(this));
            $(document).on('blur', '#scl-search-input', this.hideSuggestions.bind(this));
            $(document).on('mousedown', '.scl-suggestion-item', this.selectSuggestion.bind(this));

            // Click en card para abrir modal
            $(document).on('click', '.scl-card-item', this.openModal.bind(this));

            // Cerrar modal
            $(document).on('click', '.scl-modal-close, .scl-modal-overlay', this.closeModal.bind(this));
            $(document).on('keydown', this.handleEscKey.bind(this));

            // Formulario de solicitud
            $(document).on('submit', '#scl-solicitud-form', this.handleSolicitudSubmit.bind(this));

            // Dashboard - Editar
            $(document).on('click', '.scl-btn-edit', this.openEditModal.bind(this));
            $(document).on('click', '#scl-edit-modal .scl-modal-close, #scl-edit-modal .scl-modal-overlay', this.closeEditModal.bind(this));

            // Dashboard - Ver
            $(document).on('click', '.scl-btn-view', this.viewEstablecimiento.bind(this));

            // Formulario de edición
            $(document).on('submit', '#scl-edit-form', this.handleEditSubmit.bind(this));
        },
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        $body.html('<p class="scl-message scl-message-error">' + (response.data.message || scl_ajax.i18n.error) + '</p>');
                    }
                },
                error: function() {
                    $body.html('<p class="scl-message scl-message-error">' + scl_ajax.i18n.error + '</p>');
                }
            });
        },
        ...
        closeModal: function(e) {
            if ($(e.target).hasClass('scl-modal-overlay') || $(e.target).hasClass('scl-modal-close') || $(e.target).closest('.scl-modal-close').length) {
                $('#scl-modal').hide();
                $('body').css('overflow', '');
            }
        },
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        $message.removeClass('success').addClass('error').text(response.data.message || scl_ajax.i18n.error).show();
                    }
                },
                error: function() {
                    $message.removeClass('success').addClass('error').text(scl_ajax.i18n.error).show();
                },
                complete: function() {
                    $button.prop('disabled', false).text($button.data('original-text') || 'Enviar solicitud');
                }
            });
        },
